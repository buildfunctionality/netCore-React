...
public static class WebApplicationHouseExtensions{
    public static void MapHouseEndPoints(this WebApplication app)
    {
        ...
        app.MapPut("/house", [Authorize]async ([FromBody] HouseDetailDto dto, IHouseRepository repo) => {
        if(!MiniValidator.TryValidate(dto,out var errors))
                return Results.ValidationProblem(errors);
        ...
        app.MapDelete("/houses/{Id:int}", [Authorize]async (int Id, IHouseRepository repo) => {
